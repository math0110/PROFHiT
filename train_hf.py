...
SEED = 42
DEVICE = "cuda"
DATASET = os.environ.get("PROFHIT_DATASET", "prison")  # prison, tourism, police, m5
PRE_TRAIN_EPOCHS = 20
TRAIN_EPOCHS = 40
EVAL_SAMPLES = 100
PRE_TRAIN_LR = 0.001
...
data_obj = GroupedHierarchyData(DATASET)
print(
    f"Loaded {DATASET}: n_nodes={data_obj.n_nodes} n_leaf={data_obj.n_leaf} "
...
BACKUP_TIME = int(max(4, min(2 * data_obj.seasonality, TRAIN_UPTO // 3)))
...
hmatrices = {g: float_tensor(m) for g, m in data_obj.generate_hmatrices().items()}
...
dce_overall, dce_per_group = distributional_consistency_error(
    mean_preds.mean(axis=1), std_preds.mean(axis=1), data_obj.generate_hmatrices()
)
